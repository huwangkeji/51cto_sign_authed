var cto = {
	...
	utility : {
		...
		/**
		 * è·åå¯¹è±¡åæ ä½ç½®ï¼ç¨äºå¼¹åºå±å±ä¸­æèåç­
		 * @param options = {
		 		obj : éè¦è·åä½ç½®çå¯¹è±¡
				relative : ä½ç½®ç¸å¯¹å¯¹è±¡ï¼é»è®¤$(window)
				xType : xè½´ç¸å¯¹ä½ç½® (center,left,right)
				yType : yè½´ç¸å¯¹ä½ç½® (middle,top,bottom)
				xMargin : xè½´ä½ç½®ç©ºåº
				yMargin : yè½´ä½ç½®ç©ºåº
				limit : [ä¸,å³,ä¸,å·¦] éå¶ä½ç½®æå¤§èå´ï¼é»è®¤æç§windowçªä½å¤§å°
				limitT : éå¶æå¤§èå´ä¸
				limitR : éå¶æå¤§èå´å³
				limitB : éå¶æå¤§èå´ä¸
				limitL : éå¶æå¤§èå´å·¦
				reverse : è¶è¿èå´å¤çæ¹å¼ (true:åè½¬,false:æç§è¶è¿æ°éåç§»)ï¼é»è®¤true
			}
		 * @return {x:xåæ ,y:yåæ }
		 */
